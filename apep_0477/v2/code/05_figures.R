...
# WS3: Diff-in-disc event study figure
# WS1: Match validation figure
# WS8: Improved exhibit formatting
###############################################################################
...
placebo_file <- file.path(DATA_DIR, "placebo_cutoff_results.csv")
placebo_dt <- if (file.exists(placebo_file)) fread(placebo_file) else NULL
...
###############################################################################
# Figure 1: EPC Score Distribution with Band Boundaries (WS8: larger labels)
###############################################################################
...
p1 <- ggplot(df, aes(x = epc_score)) +
  ...
  annotate("text", x = EPC_BOUNDARIES, y = Inf, label = EPC_BAND_NAMES,
           vjust = 1.5, hjust = 0.5, fontface = "bold", size = 4.5,
           color = pal) +
  annotate("text", x = c(10, 30, 47, 62, 75, 87, 96),
           y = -Inf, vjust = -0.5, size = 4,
...
main_est <- rdd_dt[tenure == "All" & period == "Overall" & spec == "covariates"]
...
p3 <- ggplot(main_est, aes(x = estimate, y = boundary, color = boundary)) +
  ...
  labs(
    ...
    subtitle = "Robust bias-corrected estimates with clustered SEs; 95% CI"
  )
...
p4 <- ggplot(period_est, aes(x = period, y = estimate, color = boundary,
  ...
  labs(
    ...
    subtitle = "Clustered SEs at district level; vertical bars are 95% CI"
  ) +
...
if (is.null(placebo_dt)) {
  cat("  Skipping — placebo_cutoff_results.csv not found\n")
} else {
...
for (i in seq_along(EPC_BOUNDARIES)) {
  ...
  p_label <- if (saved_p < 0.001) "p < 0.001" else sprintf("p = %.3f", saved_p)
  density_plots[[i]] <- ggplot(binned_dens, aes(x = epc_score, y = N)) +
    geom_col(fill = "grey70", width = 0.8) +
    geom_vline(xintercept = b, linetype = "dashed", color = pal[i],
               linewidth = 0.8) +
    labs(
      title = sprintf("%s (%s)", bname, p_label),
      x = "EPC Score",
      y = "Count"
    ) +
...
p8 <- wrap_plots(density_plots, ncol = 3, nrow = 2) +
  plot_annotation(
...
###############################################################################
# Figure 9: WS3 — Annual Event Study (E/F and C/D RDD by Year)
###############################################################################

cat("Figure 9: Annual event study...\n")

if (file.exists(file.path(DATA_DIR, "annual_event_study.csv"))) {
  annual_dt <- fread(file.path(DATA_DIR, "annual_event_study.csv"))

  annual_dt[, boundary := factor(boundary, levels = EPC_BAND_NAMES)]

  p9 <- ggplot(annual_dt, aes(x = year_num, y = estimate, color = boundary)) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
    geom_vline(xintercept = 2018.25, linetype = "solid", color = "grey30",
               linewidth = 0.6) +
    annotate("text", x = 2018.25, y = Inf, label = "MEES\n(Apr 2018)",
             vjust = 1.2, hjust = -0.1, size = 3, color = "grey30") +
    annotate("rect", xmin = 2021.75, xmax = 2023.5, ymin = -Inf, ymax = Inf,
             alpha = 0.08, fill = "orange") +
    annotate("text", x = 2022.6, y = Inf, label = "Energy\nCrisis",
             vjust = 1.2, size = 3, color = "orange4") +
    geom_line(linewidth = 0.8) +
    geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper), size = 0.4) +
    scale_color_manual(values = pal, name = "Boundary") +
    scale_x_continuous(breaks = 2015:2023) +
    labs(
      x = "Year",
      y = "RDD Estimate at Boundary",
      title = "Annual E/F and C/D Discontinuities, 2015--2023",
      subtitle = "Vertical line: MEES enactment; shaded area: energy crisis"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))

  ggsave(file.path(FIG_DIR, "fig9_annual_event_study.pdf"), p9,
         width = 9, height = 5.5)
}

###############################################################################
# Figure 10: WS1 — Match Quality Validation
###############################################################################

cat("Figure 10: Match quality validation...\n")
...
if (file.exists(file.path(DATA_DIR, "address_match_validation.csv"))) {
  ...
  # Compare full sample vs address-matched at E/F
  full_ef <- rdd_dt[boundary == "E/F" & tenure == "All" &
                      spec == "covariates" & period != "Overall"]
  full_ef[, sample := "Full sample"]

  addr_ef <- addr_val[period != "Overall"]
  addr_ef[, sample := "Address-matched"]

  combined <- rbind(
    full_ef[, .(period, estimate, ci_lower, ci_upper, sample)],
    addr_ef[, .(period, estimate, ci_lower, ci_upper, sample)],
    fill = TRUE
  )
  combined[, period := factor(period, levels = PERIOD_LABELS)]

  p10 <- ggplot(combined, aes(x = period, y = estimate, color = sample)) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
    geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper),
                    position = position_dodge(width = 0.3), size = 0.5) +
    scale_color_manual(values = c("Full sample" = "#E41A1C",
                                   "Address-matched" = "#377EB8"),
                       name = "") +
    labs(
      x = "",
      y = "E/F RDD Estimate",
      title = "E/F Estimates: Full Sample vs Address-Matched Subsample",
      subtitle = "Address matching provides independent validation of postcode-level results"
    ) +
    theme(axis.text.x = element_text(angle = 15, hjust = 1))

  ggsave(file.path(FIG_DIR, "fig10_match_validation.pdf"), p10,
         width = 8, height = 5)
}

###############################################################################
# Figure 11: Continuous Score-Price Relationship
# Shows that prices vary smoothly with SAP score, no threshold jumps
###############################################################################

cat("Figure 11: Continuous score-price gradient...\n")

# Bin by score, compute mean log price
score_bins <- df[, .(
  mean_log_price = mean(log_price),
  n = .N
), by = epc_score][order(epc_score)]

p11 <- ggplot(score_bins[n >= 50], aes(x = epc_score, y = mean_log_price)) +
  ...
  geom_vline(xintercept = EPC_BOUNDARIES, linetype = "dashed",
             color = pal, linewidth = 0.5, alpha = 0.7) +
  annotate("text", x = EPC_BOUNDARIES, y = Inf,
           label = EPC_BAND_NAMES, vjust = 1.5, size = 3.5,
           color = pal, fontface = "bold") +
...
ggsave(file.path(FIG_DIR, "fig11_score_price_gradient.pdf"), p11,
       width = 8, height = 5)
