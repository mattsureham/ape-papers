...
The affected population and exposure: The treatment-eligible population is the ~1.8 million Venezuelan migrants who pre-registered for the ETPV across all 23 departments. The treatment variable (ETPV pre-registrations per department population) captures the department-level dose of regularization. Who is treated: departments with higher Venezuelan concentration receive a larger regularization dose. The key assumption is that this pre-determined geographic distribution — driven by border proximity and historical migration networks — is exogenous to post-2021 labor market trajectories. The GEIH department-level outcomes capture all workers in each department. The aggregate nature of the data means we cannot isolate native-specific outcomes, but it captures the equilibrium labor market effect that is policy-relevant: did regularization disrupt aggregate employment?
