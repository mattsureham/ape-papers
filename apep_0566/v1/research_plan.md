...
## Key Results (Post-Analysis)

- **CS-DiD ATT: -2.71 (SE=1.34, p=0.043)** — reform REDUCES drug OD mortality
- **Dose-response:** Abolished (-6.45) > Conviction (-1.55) > Burden (-0.51) — monotonic gradient
- **Event study:** Flat pre-trends (event times -8 to -1 near zero), growing negative post-reform effects (-11.6 at event time 5-6)
- **RI p-value: 0.056** (500 permutations)
- **LOO range: [-3.51, -1.78]** — no single state drives result
- **Bacon decomposition:** 81.3% weight from clean treated-vs-untreated comparisons
- **Heterogeneity:** High pre-reform OD rate states (-4.19***), low rate states (+1.76 n.s.)
- **TWFE: -1.57 (n.s.)** — demonstrates attenuation bias motivating CS-DiD
