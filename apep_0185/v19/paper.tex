...
\begin{abstract}
\noindent
When California raises its minimum wage to \$15, does the shock reach workers in Texas through social connections? We construct a shift-share instrument using Facebook's Social Connectedness Index weighted by destination population, capturing the \textit{breadth} of each county's connections to high-wage labor markets. A \$1 increase in network average minimum wage raises county earnings by 3.4\% and employment by 9\% (2SLS; $F > 500$). The instrument diversifies across roughly 26 effective origin-state shocks, leave-one-state-out estimates are stable, and effects strengthen monotonically as connections are restricted to distant origins---consistent with reduced attenuation bias. Placebo instruments using GDP or employment shocks produce nulls; Anderson-Rubin confidence sets exclude zero at every distance threshold. Probability-weighted exposure---ignoring destination population---shows substantially smaller effects, confirming that connection \textit{scale} matters. Job flows reveal heightened churn; migration is negligible. The channel is labor market behavior, not political feedback.
\end{abstract}
...
This paper shows that policy shocks travel socially. Between 2012 and 2022, eleven U.S.\ states raised their minimum wages above \$12 per hour while twenty states remained at the \$7.25 federal floor. These shocks did not stay within the states that enacted them. Workers in low-wage states learned about distant wage increases through friends, family, and former classmates, and this information reshaped their local labor market behavior---their reservation wages, search intensity, and bargaining power. The scale of these connections matters: a county with dense ties to millions of workers in high-wage California responds differently from a county with equally dense ties to a rural frontier state.
...
A \$1 increase in the network average minimum wage raises county-level earnings by 3.4\% and employment by 9\% (2SLS; \Cref{tab:usd}). These are market-level equilibrium multipliers---reflecting participation, search, and reallocation responses among high-connectivity counties---rather than firm-level labor demand elasticities. The employment magnitude is large, comparable to the local multipliers of 1.5--3.0 documented by \citet{moretti2011local} and \citet{klinemoretti2014}, and should be interpreted as a LATE for counties with substantial cross-state network variation. Probability-weighted exposure---ignoring the scale of connections---shows substantially smaller and statistically insignificant employment effects despite a robust first stage ($F = 290$). This divergence confirms that the breadth of connections to high-wage areas, not merely the network share directed there, drives the response.

The identification withstands extensive scrutiny. The instrument diversifies across roughly 26 effective origin-state shocks (Herfindahl = 0.04), and leave-one-state-out estimates remain stable when excluding any individual state---including California and New York, which together contribute 45\% of instrument variance. Distance-restricted instruments show effects \textit{strengthening} as connections are limited to origins beyond 200, 300, or 500 kilometers, while pre-treatment balance improves---a pattern consistent with reduced attenuation bias and inconsistent with local confounders. Placebo instruments using GDP or employment shocks through identical network weights produce null effects. Anderson-Rubin confidence sets exclude zero at every distance threshold.

The mechanism is information, not migration. Job flow data reveal that network exposure increases both hiring and separations while net job creation remains zero---heightened churn consistent with more active search and more frequent job-to-job transitions. IRS migration data show negligible responses: controlling for migration attenuates the employment coefficient by less than 5\%. Effects concentrate in high-bite industries (retail, food services) where minimum wage information is directly relevant, with null effects in low-bite sectors. A rigorous policy diffusion analysis---with governor party, trifecta status, union density, and region-by-year fixed effects---finds that network MW exposure does \textit{not} predict state-level wage floor increases. This null cleanly separates the channels: minimum wage shocks propagate through social networks via labor market behavior, not through political feedback.
...
These findings have a concrete implication for how economists model labor markets. Standard frameworks treat a worker's outside option as determined by local conditions---nearby employers, local cost of living, the state's minimum wage. Our results suggest that outside options are not local; they are network-weighted. A worker in rural Mississippi whose social network spans Los Angeles and Houston faces a different set of reference wages than a worker in the same county whose ties run to Memphis and Shreveport. Models of job search, wage bargaining, and labor market policy evaluation should account for the fact that information about distant wages reaches workers through social connections and reshapes their behavior even absent physical mobility.
