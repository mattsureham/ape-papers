...
# ============================================================================
# 4. Quarterly Workforce Indicators (QWI) via Azure Bulk Parquet
# ============================================================================

cat("\n4. Fetching QWI data from Azure (bulk Parquet)...\n")

# Source Azure helper
source("../../../../scripts/lib/azure_data.R")
con <- apep_azure_connect()

# Fetch aggregate + key industry QWI from Azure (sex=0 → all, agegrp=A00 → all ages)
# Industries: 00 (all), 44-45 (retail), 72 (accommodation/food), 52 (finance), 54 (professional)
qwi_raw <- dbGetQuery(con, "
  ...
  WHERE geo_level = 'C'
    ...
    AND sex = '0'
    AND agegrp = 'A00'
    AND industry IN ('00', '44-45', '72', '52', '54')
...
cat("  Downloaded", format(nrow(qwi_raw), big.mark = ","), "QWI observations from Azure\n")
cat("  Industries:", paste(unique(qwi_raw$industry), collapse = ", "), "\n")
cat("  Years:", min(qwi_raw$year), "-", max(qwi_raw$year), "\n")
cat("  Counties:", n_distinct(qwi_raw$county_fips), "\n")

if (nrow(qwi_raw) == 0) {
  stop("Azure QWI query returned no data. Check Azure connection and data availability.")
}
...
# ============================================================================
# 7. QWI Demographic Data from Azure (Age × Education × Sector)
# ============================================================================

cat("\n7. Fetching QWI demographic data from Azure...\n")

# The bulk QWI download (scripts/download_qwi_bulk.py) placed
# county × NAICS-sector × demographics (sa = sex×age, se = sex×education)
# Parquet files in Azure at derived/qwi/{demo}/{ind}/{state}.parquet
#
# 306 files, ~42 GB total. We query in-place via DuckDB.
# Reusing Azure connection established in Section 4.

if (apep_azure_available()) {
  # Connection 'con' already established in Section 4

  # -------------------------------------------------------------------
  # 7A. Sex × Age (sa): all industries, county level, 2012-2022
  # -------------------------------------------------------------------
  cat("  7A. Fetching QWI sex×age data (all industries)...\n")

  qwi_sa <- tryCatch({
    DBI::dbGetQuery(con, "
      SELECT geography AS county_fips, industry, sex, agegrp,
             year, quarter,
             Emp, EmpEnd, HirA, Sep, EarnS, FrmJbGn, FrmJbLs
      FROM 'az://derived/qwi/sa/ns/*.parquet'
      WHERE geo_level = 'C'
        AND ownercode = 'A05'
        AND year >= 2012 AND year <= 2022
    ...
    cat("  WARNING: Azure QWI sa query failed:", conditionMessage(e), "\n")
    NULL
  })

  if (!is.null(qwi_sa) && nrow(qwi_sa) > 0) {
    cat("  Downloaded", format(nrow(qwi_sa), big.mark = ","), "QWI sa (age) observations\n")
    cat("  Age groups:", paste(sort(unique(qwi_sa$agegrp)), collapse = ", "), "\n")
    saveRDS(qwi_sa, "../data/raw_qwi_sa.rds")
    cat("  Saved raw_qwi_sa.rds\n")
  } else {
    cat("  No QWI sa data returned from Azure\n")
  }

  # -------------------------------------------------------------------
  # 7B. Sex × Education (se): all industries, county level, 2012-2022
  # -------------------------------------------------------------------
  cat("  7B. Fetching QWI sex×education data (all industries)...\n")

  qwi_se <- tryCatch({
    DBI::dbGetQuery(con, "
      SELECT geography AS county_fips, industry, sex, education,
             year, quarter,
             Emp, EmpEnd, HirA, Sep, EarnS, FrmJbGn, FrmJbLs
    ...
    cat("  WARNING: Azure QWI se query failed:", conditionMessage(e), "\n")
    NULL
  })

  if (!is.null(qwi_se) && nrow(qwi_se) > 0) {
    cat("  Downloaded", format(nrow(qwi_se), big.mark = ","), "QWI se (education) observations\n")
    cat("  Education levels:", paste(sort(unique(qwi_se$education)), collapse = ", "), "\n")
    saveRDS(qwi_se, "../data/raw_qwi_se.rds")
    cat("  Saved raw_qwi_se.rds\n")
  } else {
    cat("  No QWI se data returned from Azure\n")
  }

  # -------------------------------------------------------------------
  # 7C. Sector-level (sa): ~20 NAICS sectors, county level, 2012-2022
  # -------------------------------------------------------------------
  cat("  7C. Fetching QWI sector-level data (~20 NAICS)...\n")
  ...
  qwi_sector <- tryCatch({
    ...
    cat("  WARNING: Azure QWI sector query failed:", conditionMessage(e), "\n")
    NULL
  })

  if (!is.null(qwi_sector) && nrow(qwi_sector) > 0) {
    cat("  Downloaded", format(nrow(qwi_sector), big.mark = ","), "QWI sector observations\n")
    cat("  NAICS sectors:", length(unique(qwi_sector$industry)), "\n")
    cat("  Sectors:", paste(sort(unique(qwi_sector$industry)), collapse = ", "), "\n")
    saveRDS(qwi_sector, "../data/raw_qwi_sector.rds")
    cat("  Saved raw_qwi_sector.rds\n")
  } else {
    cat("  No QWI sector data returned from Azure\n")
  }

  apep_azure_disconnect(con)
} else {
  cat("  Azure not configured — skipping demographic QWI fetch\n")
  cat("  Set AZURE_STORAGE_CONNECTION_STRING in .env to enable\n")
}
...
cat("  - raw_qwi_sa.rds (if Azure available)\n")
cat("  - raw_qwi_se.rds (if Azure available)\n")
cat("  - raw_qwi_sector.rds (if Azure available)\n")
