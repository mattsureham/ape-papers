...
# ============================================================================
# 14. Create Age Heterogeneity Panel
# ============================================================================

cat("\n14. Creating age heterogeneity panel...\n")

if (file.exists("../data/raw_qwi_sa.rds")) {
  qwi_sa <- readRDS("../data/raw_qwi_sa.rds")
  cat("  Loaded", format(nrow(qwi_sa), big.mark = ","), "QWI age rows\n")

  # QWI age groups: A01=14-18, A02=19-21, A03=22-24, A04=25-34,
  ...
  qwi_sa <- qwi_sa %>%
    mutate(
      ...
      sep = as.numeric(Sep),
      age_group = agegrp,
      age_label = age_labels[agegrp]
    ) %>%
    filter(!is.na(emp), emp > 0, age_group %in% names(age_labels)) %>%
    mutate(
      log_emp = log(pmax(emp, 1)),
      log_earn = log(pmax(earn, 1))
    )
  ...
  # Merge with exposure panel
  age_panel <- qwi_sa %>%
    select(county_fips, year, quarter, age_group, age_label,
           emp, earn, hira, sep, log_emp, log_earn) %>%
    left_join(exposure_panel, by = c("county_fips" = "county_fips_1", "year", "quarter")) %>%
    left_join(centroids, by = c("county_fips" = "fips")) %>%
    left_join(
      state_mw_panel %>% select(state_fips, year, quarter, own_log_mw = log_min_wage),
      by = c("state_fips", "year", "quarter")
    ) %>%
    mutate(yearq = year + (quarter - 1) / 4) %>%
    filter(!is.na(network_mw_pop))

  cat("  Age panel:", format(nrow(age_panel), big.mark = ","), "obs\n")
  cat("  Age groups:", paste(sort(unique(age_panel$age_label)), collapse = ", "), "\n")
  cat("  Counties:", n_distinct(age_panel$county_fips), "\n")

  saveRDS(age_panel, "../data/age_panel.rds")
  cat("  Saved age_panel.rds\n")
} else {
  cat("  raw_qwi_sa.rds not found — skipping age panel\n")
}

# ============================================================================
# 15. Create Education Heterogeneity Panel
# ============================================================================

cat("\n15. Creating education heterogeneity panel...\n")

if (file.exists("../data/raw_qwi_se.rds")) {
  qwi_se <- readRDS("../data/raw_qwi_se.rds")
  cat("  Loaded", format(nrow(qwi_se), big.mark = ","), "QWI education rows\n")

  # QWI education levels: E1=<HS, E2=HS/GED, E3=Some college, E4=BA+, E5=N/A(14-18)
  edu_labels <- c(
    "E1" = "Less than HS",
    "E2" = "HS / GED",
    "E3" = "Some College",
  ...
  qwi_se <- qwi_se %>%
    mutate(
      ...
      sep = as.numeric(Sep),
      edu_group = education,
      edu_label = edu_labels[education]
    ) %>%
    filter(!is.na(emp), emp > 0, edu_group %in% names(edu_labels)) %>%
    mutate(
      log_emp = log(pmax(emp, 1)),
      log_earn = log(pmax(earn, 1))
    )
  ...
  # Merge with exposure panel
  edu_panel <- qwi_se %>%
    select(county_fips, year, quarter, edu_group, edu_label,
           emp, earn, hira, sep, log_emp, log_earn) %>%
    left_join(exposure_panel, by = c("county_fips" = "county_fips_1", "year", "quarter")) %>%
    left_join(centroids, by = c("county_fips" = "fips")) %>%
    left_join(
  ...
  cat("  Education panel:", format(nrow(edu_panel), big.mark = ","), "obs\n")
  cat("  Education groups:", paste(sort(unique(edu_panel$edu_label)), collapse = ", "), "\n")
  cat("  Counties:", n_distinct(edu_panel$county_fips), "\n")

  saveRDS(edu_panel, "../data/edu_panel.rds")
  cat("  Saved edu_panel.rds\n")
} else {
  cat("  raw_qwi_se.rds not found — skipping education panel\n")
}

# ============================================================================
# 16. Create Sector Heterogeneity Panel (~20 NAICS)
# ============================================================================

cat("\n16. Creating sector heterogeneity panel...\n")

if (file.exists("../data/raw_qwi_sector.rds")) {
  qwi_sector <- readRDS("../data/raw_qwi_sector.rds")
  cat("  Loaded", format(nrow(qwi_sector), big.mark = ","), "QWI sector rows\n")
  ...
  naics_labels <- c(
    ...
    "48-49" = "Transport/Warehouse", "51" = "Information",
    "52" = "Finance/Insurance", "53" = "Real Estate",
    "54" = "Professional/Tech", "55" = "Mgmt of Companies",
    "56" = "Admin/Waste", "61" = "Education",
  ...
  qwi_sector <- qwi_sector %>%
    mutate(
      ...
      sep = as.numeric(Sep),
      sector = industry,
      sector_label = naics_labels[industry]
    ) %>%
    filter(!is.na(emp), emp > 0, sector %in% names(naics_labels)) %>%
    mutate(
      log_emp = log(pmax(emp, 1)),
      log_earn = log(pmax(earn, 1))
    )
  ...
  sector_panel <- qwi_sector %>%
    select(county_fips, year, quarter, sector, sector_label,
           emp, earn, hira, sep, log_emp, log_earn) %>%
    left_join(exposure_panel, by = c("county_fips" = "county_fips_1", "year", "quarter")) %>%
    left_join(centroids, by = c("county_fips" = "fips")) %>%
    left_join(
  ...
  cat("  Sector panel:", format(nrow(sector_panel), big.mark = ","), "obs\n")
  cat("  Sectors:", n_distinct(sector_panel$sector), "\n")
  cat("  Sector list:", paste(sort(unique(sector_panel$sector_label)), collapse = ", "), "\n")
  cat("  Counties:", n_distinct(sector_panel$county_fips), "\n")

  saveRDS(sector_panel, "../data/sector_panel.rds")
  cat("  Saved sector_panel.rds\n")
} else {
  cat("  raw_qwi_sector.rds not found — skipping sector panel\n")
}
...
if (file.exists("../data/age_panel.rds")) cat("  - age_panel.rds\n")
if (file.exists("../data/edu_panel.rds")) cat("  - edu_panel.rds\n")
if (file.exists("../data/sector_panel.rds")) cat("  - sector_panel.rds\n")
