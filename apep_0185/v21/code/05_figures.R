...
ggsave("../figures/fig1_pop_exposure_map.pdf", fig1, width = 10, height = 6, device = "pdf")
...
ggsave("../figures/fig2_prob_exposure_map.pdf", fig2, width = 10, height = 6, device = "pdf")
...
ggsave("../figures/fig3_exposure_gap_map.pdf", fig3, width = 10, height = 6, device = "pdf")
...
# ============================================================================
# Figure 11: Age Gradient Coefficient Plot (NEW v21)
# ============================================================================

cat("\nFigure 11: Age gradient coefficient plot...\n")
...
if (file.exists("../data/demo_het_results.rds")) {
  ...
  if (length(demo_het$age) > 0) {
    age_df <- do.call(rbind, lapply(demo_het$age, function(x) {
      data.frame(label = x$label, coef = x$coef, se = x$se, f_stat = x$f_stat,
                 n_obs = x$n_obs, stringsAsFactors = FALSE)
    })) %>%
      filter(!is.na(coef)) %>%
      mutate(
        ci_lower = coef - 1.96 * se,
        ci_upper = coef + 1.96 * se,
        label = factor(label, levels = c("14-18", "19-21", "22-24", "25-34",
                                          "35-44", "45-54", "55-64", "65+"))
      )

    fig11 <- ggplot(age_df, aes(x = label, y = coef)) +
      geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
      geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2, color = "#4575b4", linewidth = 0.8) +
      geom_point(size = 3, color = "#4575b4") +
      labs(
        title = "Effect of Network MW Exposure by Age Group",
        subtitle = "2SLS coefficients with 95% CIs. Same exposure, different workers.",
        x = "Age Group",
        y = "2SLS Coefficient on Network MW"
      ) +
      theme_minimal(base_size = 12) +
      theme(
        plot.title = element_text(face = "bold", size = 14),
    ...
    ggsave("../figures/fig_age_gradient.pdf", fig11, width = 8, height = 6, device = "pdf")
    cat("  Saved fig_age_gradient.pdf\n")
  } else {
    cat("  No age heterogeneity results available\n")
  }

  # ============================================================================
  # Figure 12: Education Gradient Coefficient Plot (NEW v21)
  # ============================================================================

  cat("\nFigure 12: Education gradient coefficient plot...\n")

  if (length(demo_het$education) > 0) {
    edu_df <- do.call(rbind, lapply(demo_het$education, function(x) {
      data.frame(label = x$label, coef = x$coef, se = x$se, f_stat = x$f_stat,
                 n_obs = x$n_obs, stringsAsFactors = FALSE)
    })) %>%
      filter(!is.na(coef)) %>%
      mutate(
        ci_lower = coef - 1.96 * se,
        ci_upper = coef + 1.96 * se,
        label = factor(label, levels = c("Less than HS", "HS / GED",
                                          "Some College", "BA or Higher"))
      )

    fig12 <- ggplot(edu_df, aes(x = label, y = coef)) +
      geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
      geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2, color = "#d73027", linewidth = 0.8) +
      geom_point(size = 3, color = "#d73027") +
      labs(
        title = "Effect of Network MW Exposure by Education Level",
        subtitle = "2SLS coefficients with 95% CIs. Same exposure, different workers.",
        x = "Education Level",
        y = "2SLS Coefficient on Network MW"
      ) +
    ...
    ggsave("../figures/fig_education_gradient.pdf", fig12, width = 8, height = 6, device = "pdf")
    cat("  Saved fig_education_gradient.pdf\n")
  } else {
    cat("  No education heterogeneity results available\n")
  }

  # ============================================================================
  # Figure 13: Industry/Sector Gradient Coefficient Plot (NEW v21)
  # ============================================================================

  cat("\nFigure 13: Industry gradient coefficient plot...\n")

  if (length(demo_het$sector) > 0) {
    sec_df <- do.call(rbind, lapply(demo_het$sector, function(x) {
      data.frame(label = x$label, coef = x$coef, se = x$se, f_stat = x$f_stat,
                 n_obs = x$n_obs, stringsAsFactors = FALSE)
    })) %>%
      filter(!is.na(coef)) %>%
      mutate(
        ci_lower = coef - 1.96 * se,
        ci_upper = coef + 1.96 * se,
        # Highlight MW-relevant sectors
        highlight = ifelse(label %in% c("Retail Trade", "Accommodation/Food"), "MW-intensive", "Other")
      ) %>%
      arrange(coef) %>%
      mutate(label = factor(label, levels = label))

    fig13 <- ggplot(sec_df, aes(x = label, y = coef, color = highlight)) +
      geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
      geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2, linewidth = 0.8) +
      geom_point(size = 3) +
      scale_color_manual(values = c("MW-intensive" = "#d73027", "Other" = "#4575b4"),
                         name = "") +
      coord_flip() +
      labs(
        title = "Effect of Network MW Exposure by NAICS Sector",
        subtitle = "2SLS coefficients with 95% CIs, ordered by magnitude. Red = MW-intensive sectors.",
        x = "",
        y = "2SLS Coefficient on Network MW"
      ) +
      theme_minimal(base_size = 11) +
    ...
    ggsave("../figures/fig_industry_gradient.pdf", fig13, width = 9, height = 8, device = "pdf")
    cat("  Saved fig_industry_gradient.pdf\n")
  } else {
    cat("  No sector heterogeneity results available\n")
  }

} else {
  cat("  demo_het_results.rds not found — skipping demographic figures\n")
}
