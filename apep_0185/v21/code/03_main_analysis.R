...
# ============================================================================
# PART 14: DEMOGRAPHIC HETEROGENEITY (NEW FOR v21)
# ============================================================================

cat("\n╔══════════════════════════════════════════════════════════════════╗\n")
cat("║  PART 14: DEMOGRAPHIC HETEROGENEITY                             ║\n")
cat("╚══════════════════════════════════════════════════════════════════╝\n\n")

cat("Who responds to network MW exposure? Stratified 2SLS by age, education, sector.\n")
cat("Exposure is IDENTICAL across demographics within county-quarter.\n")
cat("Only the OUTCOME varies. We test: given the same info shock, which workers respond?\n\n")

# Helper function to run stratified 2SLS
run_stratified_2sls <- function(data, group_var, group_val, group_label) {
  sub <- data %>% filter(.data[[group_var]] == group_val)
  n_obs <- nrow(sub)
  n_counties <- n_distinct(sub$county_fips)

  if (n_obs < 5000 || n_counties < 100) {
    return(list(label = group_label, n_obs = n_obs, n_counties = n_counties,
                coef = NA, se = NA, pval = NA, f_stat = NA, converged = FALSE))
  }
  ...
  # First stage F
  fs_fit <- tryCatch({
    feols(network_mw_pop ~ network_mw_pop_out_state | county_fips + state_fips^yearq,
          data = sub, cluster = ~state_fips)
  }, error = function(e) NULL)

  f_stat <- if (!is.null(fs_fit)) (coef(fs_fit)[1] / se(fs_fit)[1])^2 else NA

  if (!is.null(iv_fit)) {
    list(
      label = group_label, n_obs = n_obs, n_counties = n_counties,
      coef = coef(iv_fit)[1], se = se(iv_fit)[1],
      pval = fixest::pvalue(iv_fit)[1], f_stat = f_stat, converged = TRUE
    )
  } else {
    list(label = group_label, n_obs = n_obs, n_counties = n_counties,
         coef = NA, se = NA, pval = NA, f_stat = f_stat, converged = FALSE)
  }
}

# -------------------------------------------------------------------
# 14A. Age Heterogeneity
# -------------------------------------------------------------------

age_het_results <- list()

if (file.exists("../data/age_panel.rds")) {
  cat("14A. Age Heterogeneity\n")
  ...
  for (ag in names(age_labels)) {
    res <- run_stratified_2sls(age_panel, "age_group", ag, age_labels[ag])
    age_het_results[[ag]] <- res
    cat(sprintf("  Age %s: 2SLS = %s, F = %s, N = %s\n",
                res$label,
                if (is.na(res$coef)) "---" else sprintf("%.4f (SE: %.4f, p: %.4f)", res$coef, res$se, res$pval),
                if (is.na(res$f_stat)) "---" else sprintf("%.1f", res$f_stat),
                format(res$n_obs, big.mark = ",")))
  }
} else {
  cat("  age_panel.rds not found — skipping age heterogeneity\n")
}

# -------------------------------------------------------------------
# 14B. Education Heterogeneity
# -------------------------------------------------------------------

edu_het_results <- list()

if (file.exists("../data/edu_panel.rds")) {
  cat("\n14B. Education Heterogeneity\n")
  edu_panel <- readRDS("../data/edu_panel.rds")

  edu_labels <- c(
    "E1" = "Less than HS", "E2" = "HS / GED",
    "E3" = "Some College", "E4" = "BA or Higher"
  )

  for (ed in names(edu_labels)) {
    res <- run_stratified_2sls(edu_panel, "edu_group", ed, edu_labels[ed])
    edu_het_results[[ed]] <- res
    cat(sprintf("  Edu %s: 2SLS = %s, F = %s, N = %s\n",
                res$label,
                if (is.na(res$coef)) "---" else sprintf("%.4f (SE: %.4f, p: %.4f)", res$coef, res$se, res$pval),
                if (is.na(res$f_stat)) "---" else sprintf("%.1f", res$f_stat),
                format(res$n_obs, big.mark = ",")))
  ...
  cat("  edu_panel.rds not found — skipping education heterogeneity\n")
}

# -------------------------------------------------------------------
# 14C. Sector Heterogeneity (~20 NAICS)
# -------------------------------------------------------------------

sector_het_results <- list()

if (file.exists("../data/sector_panel.rds")) {
  cat("\n14C. Sector Heterogeneity\n")
  sector_panel <- readRDS("../data/sector_panel.rds")

  sectors <- sort(unique(sector_panel$sector))
  for (sec in sectors) {
    sec_label <- unique(sector_panel$sector_label[sector_panel$sector == sec])[1]
    res <- run_stratified_2sls(sector_panel, "sector", sec, sec_label)
    sector_het_results[[sec]] <- res
    cat(sprintf("  %s (%s): 2SLS = %s, F = %s, N = %s\n",
                sec, res$label,
                if (is.na(res$coef)) "---" else sprintf("%.4f (SE: %.4f)", res$coef, res$se),
                if (is.na(res$f_stat)) "---" else sprintf("%.1f", res$f_stat),
                format(res$n_obs, big.mark = ",")))
  }
} else {
  cat("  sector_panel.rds not found — skipping sector heterogeneity\n")
}

# Save demographic heterogeneity results
demo_het_results <- list(
  age = age_het_results,
  education = edu_het_results,
  sector = sector_het_results
)

saveRDS(demo_het_results, "../data/demo_het_results.rds")
cat("\nSaved demo_het_results.rds\n")

# Summary
cat("\n  === DEMOGRAPHIC HETEROGENEITY SUMMARY ===\n")
if (length(age_het_results) > 0) {
  age_coefs <- sapply(age_het_results, function(x) x$coef)
  cat("  Age: coefficient range [", round(min(age_coefs, na.rm = TRUE), 4), ",",
      round(max(age_coefs, na.rm = TRUE), 4), "]\n")
}
if (length(edu_het_results) > 0) {
  edu_coefs <- sapply(edu_het_results, function(x) x$coef)
  cat("  Education: coefficient range [", round(min(edu_coefs, na.rm = TRUE), 4), ",",
      round(max(edu_coefs, na.rm = TRUE), 4), "]\n")
}
if (length(sector_het_results) > 0) {
  sec_coefs <- sapply(sector_het_results, function(x) x$coef)
  sec_coefs <- sec_coefs[!is.na(sec_coefs)]
  cat("  Sector: coefficient range [", round(min(sec_coefs), 4), ",",
      round(max(sec_coefs), 4), "] across", length(sec_coefs), "converged sectors\n")
}
...
# ============================================================================
# PART 15: DYNAMIC DIAGNOSTICS — LEADS AND LAGS (NEW FOR v21)
# ============================================================================

cat("\n╔══════════════════════════════════════════════════════════════════╗\n")
cat("║  PART 15: DYNAMIC DIAGNOSTICS (LEADS/LAGS)                     ║\n")
cat("╚══════════════════════════════════════════════════════════════════╝\n\n")

cat("Testing whether effects are contemporaneous (lags significant, leads null).\n")
cat("This addresses reviewer concern about differential pre-trends.\n\n")

# Create leads and lags of the instrument (out-of-state exposure)
panel <- panel %>%
  arrange(county_fips, yearq) %>%
  group_by(county_fips) %>%
  mutate(
    # Lags (past exposure)
    network_mw_pop_out_state_L4 = dplyr::lag(network_mw_pop_out_state, 4),  # 1 year ago
    network_mw_pop_out_state_L8 = dplyr::lag(network_mw_pop_out_state, 8),  # 2 years ago
    # Leads (future exposure — should be null if no pre-trends)
    network_mw_pop_out_state_F4 = dplyr::lead(network_mw_pop_out_state, 4), # 1 year ahead
    network_mw_pop_out_state_F8 = dplyr::lead(network_mw_pop_out_state, 8)  # 2 years ahead
  ) %>%
  ungroup()

# Reduced-form regression with leads and lags
cat("15A. Reduced-form with leads and lags of the instrument:\n")

# Employment
rf_dynamic_emp <- tryCatch({
  feols(log_emp ~ network_mw_pop_out_state_F8 + network_mw_pop_out_state_F4 +
          network_mw_pop_out_state + network_mw_pop_out_state_L4 +
          network_mw_pop_out_state_L8 |
          county_fips + state_fips^yearq,
        data = panel, cluster = ~state_fips)
}, error = function(e) { cat("  Error:", e$message, "\n"); NULL })

if (!is.null(rf_dynamic_emp)) {
  cat("  Employment (reduced form):\n")
  cat("    Lead 8q (t+2yr): ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_emp)["network_mw_pop_out_state_F8"],
      se(rf_dynamic_emp)["network_mw_pop_out_state_F8"],
      fixest::pvalue(rf_dynamic_emp)["network_mw_pop_out_state_F8"]), "\n")
  cat("    Lead 4q (t+1yr): ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_emp)["network_mw_pop_out_state_F4"],
      se(rf_dynamic_emp)["network_mw_pop_out_state_F4"],
      fixest::pvalue(rf_dynamic_emp)["network_mw_pop_out_state_F4"]), "\n")
  cat("    Contemporaneous: ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_emp)["network_mw_pop_out_state"],
      se(rf_dynamic_emp)["network_mw_pop_out_state"],
      fixest::pvalue(rf_dynamic_emp)["network_mw_pop_out_state"]), "\n")
  cat("    Lag 4q  (t-1yr): ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_emp)["network_mw_pop_out_state_L4"],
      se(rf_dynamic_emp)["network_mw_pop_out_state_L4"],
      fixest::pvalue(rf_dynamic_emp)["network_mw_pop_out_state_L4"]), "\n")
  cat("    Lag 8q  (t-2yr): ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_emp)["network_mw_pop_out_state_L8"],
      se(rf_dynamic_emp)["network_mw_pop_out_state_L8"],
      fixest::pvalue(rf_dynamic_emp)["network_mw_pop_out_state_L8"]), "\n")
}

# Earnings
rf_dynamic_earn <- tryCatch({
  feols(log_earn ~ network_mw_pop_out_state_F8 + network_mw_pop_out_state_F4 +
          network_mw_pop_out_state + network_mw_pop_out_state_L4 +
          network_mw_pop_out_state_L8 |
          county_fips + state_fips^yearq,
        data = panel, cluster = ~state_fips)
...
if (!is.null(rf_dynamic_earn)) {
  cat("\n  Earnings (reduced form):\n")
  cat("    Lead 8q (t+2yr): ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_earn)["network_mw_pop_out_state_F8"],
      se(rf_dynamic_earn)["network_mw_pop_out_state_F8"],
      fixest::pvalue(rf_dynamic_earn)["network_mw_pop_out_state_F8"]), "\n")
  cat("    Lead 4q (t+1yr): ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_earn)["network_mw_pop_out_state_F4"],
      se(rf_dynamic_earn)["network_mw_pop_out_state_F4"],
      fixest::pvalue(rf_dynamic_earn)["network_mw_pop_out_state_F4"]), "\n")
  cat("    Contemporaneous: ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_earn)["network_mw_pop_out_state"],
      se(rf_dynamic_earn)["network_mw_pop_out_state"],
      fixest::pvalue(rf_dynamic_earn)["network_mw_pop_out_state"]), "\n")
  cat("    Lag 4q  (t-1yr): ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_earn)["network_mw_pop_out_state_L4"],
      se(rf_dynamic_earn)["network_mw_pop_out_state_L4"],
      fixest::pvalue(rf_dynamic_earn)["network_mw_pop_out_state_L4"]), "\n")
  cat("    Lag 8q  (t-2yr): ", sprintf("%.4f (SE: %.4f, p: %.3f)",
      coef(rf_dynamic_earn)["network_mw_pop_out_state_L8"],
      se(rf_dynamic_earn)["network_mw_pop_out_state_L8"],
      fixest::pvalue(rf_dynamic_earn)["network_mw_pop_out_state_L8"]), "\n")
}

# Joint F-test on leads
if (!is.null(rf_dynamic_emp)) {
  lead_test_emp <- tryCatch({
    wald(rf_dynamic_emp, c("network_mw_pop_out_state_F4", "network_mw_pop_out_state_F8"))
  }, error = function(e) NULL)
  if (!is.null(lead_test_emp)) {
    cat("\n  Joint F-test on leads (Employment): F =", round(lead_test_emp$stat, 2),
        ", p =", round(lead_test_emp$p, 4), "\n")
  }
}

if (!is.null(rf_dynamic_earn)) {
  lead_test_earn <- tryCatch({
    wald(rf_dynamic_earn, c("network_mw_pop_out_state_F4", "network_mw_pop_out_state_F8"))
  }, error = function(e) NULL)
  if (!is.null(lead_test_earn)) {
    cat("  Joint F-test on leads (Earnings):   F =", round(lead_test_earn$stat, 2),
        ", p =", round(lead_test_earn$p, 4), "\n")
  }
}

# Save dynamic results
dynamic_results <- list(
  rf_dynamic_emp = rf_dynamic_emp,
  rf_dynamic_earn = rf_dynamic_earn
)
saveRDS(dynamic_results, "../data/dynamic_results.rds")
cat("\nSaved dynamic_results.rds\n")


# ============================================================================
# PART 16: JOB-FLOW SUBSAMPLE ROBUSTNESS (NEW FOR v21)
# ============================================================================

cat("\n╔══════════════════════════════════════════════════════════════════╗\n")
cat("║  PART 16: JOB-FLOW SUBSAMPLE ROBUSTNESS                        ║\n")
cat("╚══════════════════════════════════════════════════════════════════╝\n\n")

cat("Re-run main 2SLS on the subsample where job flows are non-missing.\n")
cat("This addresses reviewer concern that mechanism results hold for same counties.\n\n")

# Identify job-flow subsample (where hire_rate is non-missing)
if ("hire_rate" %in% names(panel)) {
  jf_subsample <- panel %>% filter(!is.na(hire_rate))
  cat("Job-flow subsample: N =", format(nrow(jf_subsample), big.mark = ","),
      "of", format(nrow(panel), big.mark = ","), "total\n")

  # 2SLS on job-flow subsample — Employment
  iv_jf_emp <- tryCatch({
    feols(log_emp ~ 1 | county_fips + state_fips^yearq |
            network_mw_pop ~ network_mw_pop_out_state,
          data = jf_subsample, cluster = ~state_fips)
  }, error = function(e) NULL)

  # 2SLS on job-flow subsample — Earnings
  iv_jf_earn <- tryCatch({
    feols(log_earn ~ 1 | county_fips + state_fips^yearq |
            network_mw_pop ~ network_mw_pop_out_state,
          data = jf_subsample, cluster = ~state_fips)
  }, error = function(e) NULL)
  ...
  f_stat_jf <- if (!is.null(fs_jf)) (coef(fs_jf)[1] / se(fs_jf)[1])^2 else NA

  cat("\nJob-flow subsample results:\n")
  if (!is.null(iv_jf_emp)) {
    cat(sprintf("  Employment: 2SLS = %.4f (SE: %.4f, p: %.4f)\n",
        coef(iv_jf_emp)[1], se(iv_jf_emp)[1], fixest::pvalue(iv_jf_emp)[1]))
  }
  if (!is.null(iv_jf_earn)) {
    cat(sprintf("  Earnings:   2SLS = %.4f (SE: %.4f, p: %.4f)\n",
        coef(iv_jf_earn)[1], se(iv_jf_earn)[1], fixest::pvalue(iv_jf_earn)[1]))
  }
  cat(sprintf("  First Stage F: %.1f\n", f_stat_jf))
  cat(sprintf("  Full sample Employment:     %.4f (SE: %.4f)\n",
      coef(iv_2sls_pop)[1], se(iv_2sls_pop)[1]))
  cat("  Difference shows sensitivity to QWI suppression selection.\n")

  # Test if suppression is correlated with instrument
  panel$jf_missing <- as.integer(is.na(panel$hire_rate))
  suppress_test <- tryCatch({
    feols(jf_missing ~ network_mw_pop_out_state | state_fips^yearq,
          data = panel, cluster = ~state_fips)
  }, error = function(e) NULL)

  if (!is.null(suppress_test)) {
    cat(sprintf("\n  Suppression-instrument correlation: coef = %.4f (SE: %.4f, p: %.3f)\n",
        coef(suppress_test)[1], se(suppress_test)[1], fixest::pvalue(suppress_test)[1]))
    if (fixest::pvalue(suppress_test)[1] > 0.05) {
      cat("  → Suppression is NOT significantly correlated with the instrument (good).\n")
    } else {
      cat("  → WARNING: Suppression IS correlated with the instrument.\n")
    }
  }

  jf_subsample_results <- list(
    iv_emp = iv_jf_emp, iv_earn = iv_jf_earn,
    f_stat = f_stat_jf, n_obs = nrow(jf_subsample),
    suppress_test = suppress_test
  )
  saveRDS(jf_subsample_results, "../data/jf_subsample_results.rds")
  cat("\nSaved jf_subsample_results.rds\n")
} else {
  cat("  hire_rate not found in panel — skipping\n")
}
