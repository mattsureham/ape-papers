...
# ============================================================================
# Table 14: Age Heterogeneity (NEW v21)
# ============================================================================

cat("\nTable 14: Age heterogeneity...\n")

if (file.exists("../data/demo_het_results.rds")) {
  demo_het <- readRDS("../data/demo_het_results.rds")

  if (length(demo_het$age) > 0) {
    age_rows <- do.call(rbind, lapply(demo_het$age, function(x) {
      data.frame(
        label = x$label, coef = x$coef, se = x$se, pval = x$pval,
        f_stat = x$f_stat, n_obs = x$n_obs, stringsAsFactors = FALSE
      )
    })) %>% filter(!is.na(coef))
    ...
    if (nrow(age_rows) > 0) {
      ...
      for (i in seq_len(nrow(age_rows))) {
        r <- age_rows[i, ]
        tab14_lines <- c(tab14_lines,
          paste0(r$label, " & ",
                 sprintf("%.4f", r$coef), stars(r$pval), " & ",
                 sprintf("(%.4f)", r$se), " & ",
                 sprintf("%.1f", r$f_stat), " & ",
      ...
      writeLines(tab14_lines, "../tables/tab_age_het.tex")
      cat("  Saved tab_age_het.tex\n")
    }
  }

  # ============================================================================
  # Table 15: Education Heterogeneity (NEW v21)
  # ============================================================================

  cat("\nTable 15: Education heterogeneity...\n")

  if (length(demo_het$education) > 0) {
    edu_rows <- do.call(rbind, lapply(demo_het$education, function(x) {
      data.frame(
        label = x$label, coef = x$coef, se = x$se, pval = x$pval,
        f_stat = x$f_stat, n_obs = x$n_obs, stringsAsFactors = FALSE
      )
    ...
    if (nrow(edu_rows) > 0) {
      ...
      for (i in seq_len(nrow(edu_rows))) {
        r <- edu_rows[i, ]
        tab15_lines <- c(tab15_lines,
          paste0(r$label, " & ",
                 sprintf("%.4f", r$coef), stars(r$pval), " & ",
                 sprintf("(%.4f)", r$se), " & ",
                 sprintf("%.1f", r$f_stat), " & ",
      ...
      writeLines(tab15_lines, "../tables/tab_edu_het.tex")
      cat("  Saved tab_edu_het.tex\n")
    }
  }

  # ============================================================================
  # Table 16: Sector Heterogeneity (~20 NAICS) (NEW v21)
  # ============================================================================

  cat("\nTable 16: Sector heterogeneity...\n")

  if (length(demo_het$sector) > 0) {
    sec_rows <- do.call(rbind, lapply(demo_het$sector, function(x) {
      data.frame(
        label = x$label, coef = x$coef, se = x$se, pval = x$pval,
        f_stat = x$f_stat, n_obs = x$n_obs, stringsAsFactors = FALSE
      )
    })) %>% filter(!is.na(coef)) %>% arrange(desc(coef))
    ...
    if (nrow(sec_rows) > 0) {
      ...
      writeLines(tab16_lines, "../tables/tab_sector_het.tex")
      cat("  Saved tab_sector_het.tex\n")
    }
  }
} else {
  cat("  demo_het_results.rds not found — skipping demographic tables\n")
}
