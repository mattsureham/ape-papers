...
\begin{abstract}
\noindent
When California raises its minimum wage to \$15, does the shock reach workers in Texas---not through policy imitation, but through social connections? We construct a population-weighted measure of network minimum wage exposure using Facebook's Social Connectedness Index, capturing the \textit{breadth} of each county's connections to high-wage labor markets. Using out-of-state network ties as instruments ($F > 500$), we find that network minimum wage exposure significantly raises local earnings and employment. Effects strengthen monotonically with instrument distance, placebo shocks produce nulls, and Anderson-Rubin sets exclude zero. A built-in specification test reveals that probability-weighted exposure---ignoring destination population---shows substantially smaller employment effects, confirming that the \textit{scale} of connections drives the response. Job flows reveal heightened churn; migration is negligible. Policy diffusion analysis with political controls shows a null: the channel is labor market behavior, not political feedback.
\end{abstract}
...
The joint F-test of pre-period event-study coefficients yields $p = 0.007$, which warrants careful examination. This rejection reflects baseline \textit{level} differences in employment across exposure quartiles (\Cref{tab:balance}), not differential \textit{trends}. The pre-period coefficients are individually small (all within 0.05 of zero) and statistically insignificant, but the joint test has power against the non-monotonic level variation documented in the balance table ($p = 0.004$). Critically, county fixed effects absorb all time-invariant level differences, so identification relies entirely on within-county variation over time---these level differences do not bias the estimator. The reduced-form pre-trend F-test produces a similar pattern, and visual inspection of the pre-period event-study path (\Cref{fig:event_study}) shows no evidence of a pre-existing trend or anticipatory response.
...
The results are unambiguous: network minimum wage exposure does not predict future state-level MW adoption. The baseline coefficient is negative and statistically insignificant ($\hat{\beta} = -0.54$, $p = 0.30$). Adding political controls leaves the coefficient insignificant ($p = 0.25$), as does adding union density and unemployment ($p = 0.61$). The IV specification (Column~5) produces a meaningless estimate with a first-stage $F$-statistic of 0.9---far below standard thresholds---indicating that non-adjacent network exposure has no power to predict full network exposure at the state level. The continuous outcome confirms the same pattern: no specification produces a positive and significant coefficient.
