...
\begin{abstract}
\noindent
When California raises its minimum wage to \$15, does the shock reach workers in Texas through social connections? We build a population-weighted exposure measure from Facebook's Social Connectedness Index, capturing the \textit{breadth} of each county's ties to high-wage labor markets, and instrument with out-of-state connections. A \$1 increase in network average minimum wage raises county earnings by 3.4\% and employment by 9\%. Population-weighted exposure---which accounts for the \textit{scale} of connections---substantially outperforms probability-weighted exposure, confirming that the number of potential contacts in high-wage areas, not merely the network share directed there, drives the response. Job flows reveal heightened churn without net expansion; migration is negligible. The evidence is consistent with information transmission about wages rather than relocation or political feedback. Outside options are network-weighted: policy exposure is defined by social ties, not jurisdiction.
\end{abstract}
...
Three contributions emerge. First, and most broadly, we establish that workers' outside options are network-weighted---a general result with minimum wages as the application. Standard models of job search and wage bargaining assume local outside options; we show that socially transmitted wage information from distant labor markets materially affects local equilibria \citep{jager2024worker}. Second, we introduce a population-weighting methodology for SCI-based exposure measures, demonstrating that connection \textit{scale}---not just connection probability---is the empirically relevant margin, with implications for the growing body of research using shift-share designs with social network weights \citep{bailey2018social, bailey2018house, chetty2022social}. Third, we contribute to the literature on how networks transmit information rather than people \citep{kramarz2023, faberman2022}, showing that workers update their labor market behavior based on wage signals from distant connections even absent physical migration, over distances far exceeding the geographic spillovers studied by \citet{dube2014designing}.
...
If network connections transmit wage information that shifts workers' expectations and bargaining behavior, a natural downstream question is whether this information also shapes the political environment: does exposure to distant minimum wage increases make a state more likely to raise its own wage floor? We investigate this hypothesis rigorously using a state-year panel with progressive controls.

For each state-year (51 states, 2012--2021), we construct the state-level population-weighted network minimum wage exposure by aggregating county-pair SCI to state-state weights: $\text{SCI}_{ss'} = \sum_{c \in s, d \in s'} \text{SCI}_{cd} \times \text{Pop}_d$, normalized so weights sum to one. The outcome is whether state $s$ raised its minimum wage in year $t+1$ (linear probability model) or the magnitude of the change ($\Delta\log(\text{MW})_{s,t+1}$). States at the \$15 ceiling are excluded, yielding 510 state-year observations with 163 MW increases (32\%).

\Cref{tab:diffusion} reports five specifications with progressively demanding controls. Column~1 includes only lagged own MW, state, and year fixed effects. Column~2 adds governor party and trifecta status. Column~3 adds union density and the state unemployment rate. Column~4 replaces year fixed effects with Census region $\times$ year fixed effects. Column~5 instruments full network exposure with exposure computed using only non-adjacent states (those not sharing a border).

The results provide no evidence that network exposure promotes future MW adoption. The baseline coefficient is negative and statistically insignificant ($\hat{\beta} = -0.54$, $p = 0.30$). Adding political controls leaves the coefficient insignificant ($p = 0.25$), as does adding union density and unemployment ($p = 0.61$). The most demanding OLS specification (Column~4), which replaces year fixed effects with region$\times$year fixed effects, yields a significant \textit{negative} coefficient ($\hat{\beta} = -1.34$, $p = 0.03$)---suggesting that states with high network exposure to distant MW increases are, if anything, \textit{less} likely to raise their own wages, possibly reflecting political backlash or ceiling effects in adoption-prone regions. The IV specification (Column~5) produces a meaningless estimate with a first-stage $F$-statistic of 0.9---far below standard thresholds---indicating that non-adjacent network exposure has no power to predict full network exposure at the state level. The continuous outcome ($\Delta\log$ MW) confirms the same pattern: no specification produces a positive and significant coefficient.

\input{tables/tab_policy_diffusion}

Falsification tests support the null. Network exposure to gas tax rates and corporate income tax rates---computed using the same SCI weights---shows no predictive power for MW adoption (gas tax: $p = 0.89$; corporate tax: $p = 0.64$). In a horse race between SCI-weighted and inverse-distance-weighted MW exposure, neither channel is significant, confirming that the null reflects a genuine absence of the policy diffusion channel rather than a measurement problem.

This result is informative. Network exposure does not promote MW adoption; if anything, the most demanding specification suggests a deterrent effect, consistent with political backlash or ceiling effects in adoption-prone regions. It establishes that the labor market effects documented in this paper---earnings, employment, job flows---operate through a direct information channel that is independent of any political feedback mechanism. Workers update their reservation wages and search behavior based on network information about distant wages, but this behavioral response does not translate into downstream political pressure for policy change. The channels through which minimum wage shocks propagate through social networks are economic, not political.
...
The results confirm this prediction sharply: the 2SLS employment effect is concentrated in high-bite industries, with the low-bite coefficient small and statistically insignificant ($p > 0.10$). This industry heterogeneity is among the paper's cleanest tests of the information mechanism. If the results were driven by a generic confounding factor---correlated labor demand shocks, regional economic trends, or unobserved amenity changes---we would expect similar effects across industries. Instead, effects appear precisely where minimum wage information is most relevant to workers' labor market decisions: the sectors where wages cluster near the minimum wage floor, and where workers are most likely to revise their reservation wages upon learning that peers in distant states earn \$15 per hour. The null in professional services and finance---sectors where wages far exceed any minimum wage---rules out a broad ``connected to a booming economy'' confound and isolates the minimum-wage-specific information channel.
...
The mechanism is information, not migration. Job flows reveal heightened churn---more hiring and more separations---without net expansion. Migration responses are negligible, accounting for less than 5\% of the employment effect. Effects concentrate in high-bite industries where minimum wage information directly shapes workers' decisions. A rigorous policy diffusion analysis with progressive political controls finds no positive relationship between network exposure and state-level minimum wage adoption---the most demanding specification yields a significant \textit{negative} coefficient---establishing that the labor market channel operates independently of any political feedback mechanism.
...
\section{Policy Diffusion: Additional Results}
\label{sec:appendix_diffusion}

\input{tables/tab_diffusion_falsification}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_diffusion_distance.pdf}
\caption{Policy Diffusion: Distance Monotonicity}
\label{fig:diffusion_distance}
\begin{figurenotes}
Coefficient on network MW exposure in the LPM for state-level MW adoption, estimated at progressively restrictive distance thresholds. At each threshold, only connections between states with population centroids separated by at least the given distance contribute to the network exposure measure. Shaded region shows 95\% confidence intervals. All specifications include political controls (governor party, trifecta, union density, unemployment), lagged own MW, state FE, and year FE.
\end{figurenotes}
\end{figure}
