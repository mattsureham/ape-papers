...
## APEP-0642 v3: Regulatory Whack-a-Mole
...
# Use Air + average of Water/Land/POTW (NonAir has collinear SEs)
es_nonair_avg <- es_coefs[medium %in% c("Water", "Land", "POTW"),
                          .(estimate = mean(estimate), se = mean(se), pvalue = mean(pvalue)),
                          by = event_time]
es_nonair_avg[, medium := "NonAir"]
es_main <- rbind(es_coefs[medium == "Air"], es_nonair_avg, fill = TRUE)
...
# Extract coefficients from harmonized mechanism models (V3 reparameterized)
mech_data <- data.table(
  chemical = rep(c("CAA-Regulated", "Non-CAA"), each = 2),
  term = rep(c("Common Post (θ)", "Air Differential (τ)"), 2),
  estimate = c(coef(models$m_caa)["post"],
               coef(models$m_caa)["post_air"],
               coef(models$m_noncaa)["post"],
               coef(models$m_noncaa)["post_air"]),
  se = c(se(models$m_caa)["post"],
         se(models$m_caa)["post_air"],
         se(models$m_noncaa)["post"],
         se(models$m_noncaa)["post_air"])
)
...
coef_compare <- data.table(
  ...
  estimate = c(
    coef(models$medium_results$Air)["post"],
    coef(models$medium_results$Water)["post"],
    coef(models$medium_results$Land)["post"],
    coef(models$medium_results$POTW)["post"],
    coef(models$medium_results$Air)["post"],
    coef(models$medium_results$Water)["post"],
    coef(models$medium_results$Land)["post"],
    coef(models$medium_results$POTW)["post"]
  ...
  se = c(
    se(models$medium_results$Air)["post"],
    se(models$medium_results$Water)["post"],
    se(models$medium_results$Land)["post"],
    se(models$medium_results$POTW)["post"],
    se(models$medium_results$Air)["post"],
    se(models$medium_results$Water)["post"],
    se(models$medium_results$Land)["post"],
    se(models$medium_results$POTW)["post"]
...
if (file.exists(ri_file)) {
  ...
  actual_coef <- coef(models$m_reparam_cwa)["post_nonair"]
