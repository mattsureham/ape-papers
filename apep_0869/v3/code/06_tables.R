...
# APEP-0869 V3: Enforcement Design and Industry Adjustment
...
# State x Quarter FE — VCOV may be non-PSD with 6 clusters; report point estimate only
ct_sq <- coeftable(m_state_qtr)["triple_cont", ]
# Check for pathological SE (near-zero from non-PSD VCOV)
if (ct_sq[2] < 0.001) {
  cat(sprintf("State $\\times$ Quarter FE & %.3f & [a] & [a] \\\\\n", ct_sq[1]))
} else {
  p_sq <- ct_sq[4]
...
cat("[a] VCOV matrix not positive semi-definite with six state clusters and state $\\times$ quarter FE; ")
cat("standard errors and $p$-values are unreliable. Point estimate reported for transparency only.\n")
cat("\\end{tablenotes}\n")
cat("\\end{table}\n")
sink()
