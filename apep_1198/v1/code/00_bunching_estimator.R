...
## apep_1198: Adapted from apep_0727 v4 for UK FIT thresholds
##
...
##
## NOTE: This estimator works in 0.1 kW bins (integer-coded as kW * 10).
## For 4 kW threshold: kink_int = 40L, excl_lower = 35L, excl_upper = 45L
## For 10 kW threshold: kink_int = 100L, excl_lower = 90L, excl_upper = 110L
## For 50 kW threshold: kink_int = 500L, excl_lower = 450L, excl_upper = 550L
...
bootstrap_bunching_int <- function(dt_raw,
                                    ...
                                    n_boot = 500L,
                                    subset_expr = NULL) {
  dt_sub <- dt_raw[get(capacity_col) >= (window_lower / 10) &
                    get(capacity_col) < ((window_upper + 1) / 10)]
  ...
  for (b_idx in 1:n_boot) {
    ...
    boot_bins <- boot_dt[, .(count = .N),
                          by = .(bin_int = as.integer(floor(get(capacity_col) * 10)))]
...
make_bins_int <- function(dt_sub, capacity_col = "capacity_kw",
                           window_lower = 10L, window_upper = 70L) {
  all_bins <- data.table(bin_int = seq(window_lower, window_upper, by = 1L))
  bins <- dt_sub[, .(count = .N),
                  by = .(bin_int = as.integer(floor(get(capacity_col) * 10)))]
