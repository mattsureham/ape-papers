...
# -----------------------------------------------------------
# 5b. Alternative pastoral classifications (Fix 3)
# -----------------------------------------------------------
cat("Building alternative pastoral classifications...\n")

# (A) Geography-only: Middle Belt + GLW cattle density, NO violence history
pastoral_geo_lgas <- c()

# Middle Belt LGAs
mb_lgas_geo <- nga_lgas$GID_2[nga_lgas$NAME_1 %in% middle_belt_states]
pastoral_geo_lgas <- mb_lgas_geo

# Add GLW cattle density if available
if (file.exists(glw_file)) {
  pastoral_geo_lgas <- unique(c(pastoral_geo_lgas, pastoral_lgas_glw))
}

cat(sprintf("Pastoral-geo LGAs (geography only, no violence): %d\n",
            length(pastoral_geo_lgas)))

# (B) Conflict-only: Pre-treatment violence only, NO geography
pastoral_conflict_lgas <- pre_treatment_events %>%
  group_by(lga_id) %>%
  summarise(pre_events = n(), .groups = "drop") %>%
  filter(pre_events >= 2) %>%
  pull(lga_id)

cat(sprintf("Pastoral-conflict LGAs (conflict only, no geography): %d\n",
            length(pastoral_conflict_lgas)))
...
lga_panel <- lga_panel %>%
  ...
  mutate(
    ...
    pastoral_geo = as.integer(lga_id %in% pastoral_geo_lgas),
    pastoral_conflict = as.integer(lga_id %in% pastoral_conflict_lgas),
