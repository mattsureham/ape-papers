...
if (!file.exists(glw_file)) {
  tryCatch({
    ...
    # Verify it's a valid GeoTIFF (should be >100KB)
    if (file.size(glw_file) < 100000) {
      cat("WARNING: GLW file too small (likely redirect page). Removing.\n")
      file.remove(glw_file)
    } else {
    ...
    if (file.exists(glw_file)) file.remove(glw_file)
  })
} else {
