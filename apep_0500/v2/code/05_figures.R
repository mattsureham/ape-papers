...
# -----------------------------------------------------------
# Figure 8: Within-State Displacement Event Study (Fix 2)
# -----------------------------------------------------------
cat("Figure 8: Displacement event study...\n")

np_es_df <- tryCatch(
  readRDS(file.path(data_dir, "displacement_event_study.rds")),
  error = function(e) NULL
)

if (!is.null(np_es_df) && nrow(np_es_df) > 0) {
  fig8 <- ggplot(np_es_df, aes(x = e, y = att)) +
    geom_hline(yintercept = 0, color = "gray30", linewidth = 0.8) +
    geom_vline(xintercept = -0.5, linetype = "dashed", color = "red", linewidth = 0.5) +
    geom_errorbar(aes(ymin = ci_lo, ymax = ci_hi),
                  width = 0.2, color = "gray40", linewidth = 0.6) +
    ...
    labs(
      x = "Periods relative to law adoption",
      y = "Coefficient (non-state violence events)",
      title = "Within-State Displacement Test: Non-Pastoral LGAs in Treated States",
      subtitle = "Should be flat at zero if deterrence; positive if displacement"
    ) +
    theme(axis.title = element_text(size = 11),
          plot.subtitle = element_text(size = 9))

  ggsave(file.path(fig_dir, "fig8_displacement_es.pdf"), fig8,
         width = 8, height = 5, device = cairo_pdf)
  cat("Figure 8 saved.\n")
} else {
  cat("No displacement event study data available, skipping Figure 8.\n")
}
